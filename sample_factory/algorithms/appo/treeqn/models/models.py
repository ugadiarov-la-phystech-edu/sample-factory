...
class TreeQNPolicy(nn.Module):
    ...
    def forward(self, encoded_observation):
        """
        ...
        """
        Q, tree_result = self.planning(encoded_observation)

        if self.use_actor_critic:
            V = self.ac_value_fn(encoded_observation).squeeze()
        else:
            V = torch.max(Q, 1)[0]

        return V.unsqueeze(1) #Q, V, tree_result

    def value(self, encoded_observation):
        _, V, _ = self.forward(encoded_observation)
    ...
    def tree_planning(self, x, return_intermediate_values=True):
        ...
        for i in range(self.tree_depth):
            ...
            x = x.contiguous().view(-1, self.embedding_dim)
